...
class mainFormDlg(QDialog) :
    ...
    def loginClicked(self):
        data=[45,self.parent().username,self.parent().password,self.nameEdit.text(),self.gameId]
    ...
    def __init__(self, parent= None) :
        ...
        self.setWindowTitle('Create Combat')
        self.centerOnScreen()

        self.gameId = self.parent().currentGameId

        self.nameEdit = QLineEdit()
        self.submitButton = QPushButton("Create Combat")

        self.submitButton.clicked.connect(self.loginClicked)

        self.mainLayout = QFormLayout()

        self.mainLayout.addRow("Combat Name:",self.nameEdit)
